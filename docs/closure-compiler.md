...
> TL;DR read the section on our [type system](#mdc-web-type-system) and our [conventions](#mdc-web-closure-conventions).
...
seamlessly for all Google applications_. This is an absolute requirement for us.
...
1. https://github.com/google/closure-compiler/wiki/Annotating-Types
...
Defining these as enums allows the use of their properties without worrying about them being renamed
by closure. Furthermore, it prevents errors caused by potential typos in a property name.
...
`defaultAdapter` must return an object with the correct adapter shape, type-cast to the type for
the adapter.
...
export default class MDCComponentFoundation extends MDCFoundation {
  static get defaultAdapter() {
    return /** @type {!MDCComponentAdapter} */ ({
      addClass: () => {},
      removeClass: () => {},
    });
  }
...
- Use `Object<string, T>` for objects where _the type for every must be the same_.
